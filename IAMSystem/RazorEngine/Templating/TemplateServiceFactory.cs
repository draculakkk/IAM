namespace RazorEngine.Templating
{
    ...
    public static class TemplateServiceFactory
    {
        ...
        public static TemplateService CreateTemplateService(Language language = Language.CSharp, bool strictMode = false, HtmlMarkupParser markupParser = null)
        {
        ...
        private static HtmlMarkupParser CreateMarkupParser(string typeName)
        {
            Type parserType = Type.GetType(typeName);
            if (parserType == null)
                throw new ConfigurationErrorsException("The parser type '" + typeName + "' could not be loaded.");

            HtmlMarkupParser parser = Activator.CreateInstance(parserType) as HtmlMarkupParser;
            if (parser == null)

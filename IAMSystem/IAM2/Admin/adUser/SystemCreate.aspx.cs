...
{
    public partial class SystemCreate : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                txtPartmentOutDate.Attributes.Add("readonly", "true");
                ControlButton();
        ...
        void EditQuery()
        {
            ...
            if (mod != null)
            {
                var accmaping = new AccountMapingDAL().GetOneByzhanghao(mod.Accountname, "系统", "AD");
                if (accmaping == null)
                    accmaping = new AccountMapingDAL().GetOneByzhanghao(mod.Accountname, "员工", "AD");
                if (accmaping == null)
                    accmaping = new AccountMapingDAL().GetOneByzhanghao(mod.Accountname, "其他", "AD");
                if (accmaping != null)
        ...
        {
            string Employeecode = string.IsNullOrEmpty(txtNumber.Text) ? "" : txtNumber.Text.Trim();
            if (IsUpdate == true && string.IsNullOrEmpty(Employeecode))
            {
                return;
            }
            ...
            if (EmployeeModel != null)
            {
                ...
                //var en1 = new AccountMapingDAL().GetOne(Employeecode, Unitity.SystemType.AD.ToString(), "员工");
                //if (en1 == null)
                //{
                //    QueryPage("yes");
                //    return;
        ...
        void QueryGroup()
        {
            ...
            var zhiji = dplZhiji.SelectedValue;
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (txtNumber.Text.Trim() != string.Empty)
            {
                ...
                if (EmployeeModel != null)
                {
                    //var en = new AccountMapingDAL().GetOne(Employeecode, Unitity.SystemType.AD.ToString(), "系统");
                    //if (en != null && IsUpdate == false)
                    //{
                    //    QueryPage("2");
                    //    return;
            ...
            if (IsUpdate == false)
            {
                if (new ADUserInfoDAL().Add(AdEntity()) > 0)
                {
                    ...
                    BLL.ActionLog.CreateLog(base.UserInfo.adname, tmp.zhanghao, Unitity.SystemType.AD.ToString(), "系统");
...
VALUES  ( NEWID() ,'" + txtLoginName.Text.Trim() + "' ,'" + i.GroupName + "' ,0) \n");
            ...
            else
            {
                ...
                if (new ADUserInfoDAL().UpdateUserInfo(entity) > 0)
                {
                    ...
                    new BLL.ActionLog().EditLog<AD_UserInfo>(base.UserInfo.adname, entity.Accountname, Unitity.SystemType.AD.ToString(), "系统", old, entity);
                    var list = (List<AD_UserWorkGroup>)ViewState["userwork"];
                    var addlis = list.Where(item => item.isdr == 2).ToList();
...
VALUES  ( NEWID() ,'" + acc + "' ,'" + i.GroupName + "' ,0) \n");
                    ...
                    stb.Append(@"update AccountMaping set usertype='系统', gonghao='" + txtNumber.Text.Trim() + "' where type='AD' and zhanghao='" + acc + "'");
                    using (IAMEntities db = new IAMEntities())
                    {
                    ...
                    if (!chkEnable.Checked)
                    {
                        using (IAMEntities db = new IAMEntities())
                        {
                            ...
                            db.ExecuteStoreCommand("delete dbo.AD_UserWorkGroup where uid='" + AdEntity().Accountname + "'");
                            db.SaveChanges();
                        }
        ...
        protected void btnShengcheng_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(n))
            {
                ...
                if (narr.Length == 1)
                    n = narr[0];
                if (uli.FirstOrDefault(item => item.GroupName == n && item.isdr != 1) == null)
                {
